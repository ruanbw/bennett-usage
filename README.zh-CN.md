...
- **环比不说谎**：每个范围都与紧邻的**等长**窗口相比（今日对昨天同一时刻、24 小时对前一个滚动 24 小时）。没有可比窗口、或当期没有任何可缓存 Token 时，界面直接说明，而不是给数字：不会因为历史不足就报 -97%，也不会把「未测量」画成 0% 的缓存命中率。
- **快捷键**：⌘1–⌘5 切换时间范围，⌘D 打开看板，⌘, 打开设置，⌘R 立即同步。
- **设置**：通用（语言、外观、自动刷新频率、自动检查更新）、Agent 状态（日志路径状态、记录数、重新检测）、计价与汇率（USD / CNY、自定义汇率）、数据与存储（数据库位置与实际占用、一键清空）、关于（检查更新）。
...
./scripts/package-dmg.sh 1.6.1      # 指定版本号
