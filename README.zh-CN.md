...
macOS 菜单栏常驻的 AI coding 助手 Token 用量统计，聚合 **16 个工具**：Claude Code、OpenAI Codex、Continue CLI、Gemini CLI、Goose、Qwen Code、OpenCode、Roo Code / Cline / Kilo（VS Code 扩展）、Cline（桌面版 / CLI）、DSH Harness、Oh My Pi、Pi Agent、Antigravity。Cursor、GitHub Copilot、Trae 显示安装状态（云端计费，取数需 API）。
...
- **16 个数据源**：本地解析各工具会话记录，无需 API Key，纯本地 SQLite 存储，FSEvents 文件监听自动同步（Cursor / Copilot / Trae 为云端计费，仅检测安装状态）。
