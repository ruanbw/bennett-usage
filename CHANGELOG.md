...
这是一次**发布前审计**的结果，而不是又一次被动修 bug。1.6.0 把颜色、排版与信息架构统一了，但「界面上的数字是否与数据一致」从未系统检查过，于是同一类缺陷反复由用户在使用中发现。本次开了 5 个并行的只读审计（Dashboard 数值、弹窗与状态栏、设置五个分页、排版与文案、数据层不变量），逐个显示值追到它的查询与分母，并用数据库独立核对；共修掉 36 处，其中 10 处会显示出与数据库不符的数字。排版类缺陷不是靠肉眼判断，而是用 CoreText 按代码里真实字体的逐字测宽（含 `NSHostingView.fittingSize` 实测分段控件）。

### 会说谎的数字

- **24 小时范围的环比基准是 48 小时**（`fetchComparisonPeriod`）：取值来自原始记录的滚动 24 小时，基准却取自按天聚合的**两个自然日**。实测本机 `-81%`，而同样长度的诚实基准是 `-76.5%`，偏差随日内分布放大。现改为同长度滚动窗口（`fetchPeriodTotals(sinceTimestamp:untilTimestamp:)`，新增半开区间支持）。
- **「今日」环比是「今天到目前」对「整天昨天」**：于是每天早上都报大幅下降，而该类型自己的注释早就警告过这一点。现改为**昨天同一时刻截止**的窗口。
- **7 天与 30 天的环比窗口错位一天**：当前窗口是 `D-6…D0`，前序窗口却取 `D-14…D-8`，既漏掉紧邻的 `D-7` 又混入 `D-14`。现为 `D-13…D-7` 与 `D-59…D-30`。
- **部分覆盖的环比分支永不可达**：`totalTokens` 只要有一天可比就非 nil，所以「Only X days of comparison history」永远没有机会渲染，30 天窗口在 3 天历史上直接显示约 `-97%`。现在部分覆盖优先于数字呈现。
- **图表丢掉最老的一小时**：SQLite 整数除法向零截断，窗口内第一段不足一小时的记录算出 `hourIndex = -24`，索引 `-1` 被丢弃 —— 柱状图之和比同一屏的总量少最多一小时（约 4%）。现已钳制进最老的一柱。
- **弹窗缓存环是写死的 0**（`public var cacheHitRate: Double { 0 }`）：注释自称「诚实的未测量」，实际画出一个满环加琥珀色告警点，读起来是「缓存失效」，而同一时刻 Dashboard 显示 98%。现接上真实命中率，且**只有 cache read 才算命中**（rollup 的 `cache_tokens` 是读+写，用它当分子另一个代理一眼就抓到了）；无缓存读时显示「—」。
- **记录数用 `%@` 接收 `Int`**：115 107 条记录渲染成 `-383993491 条记录`。同一文件里其余计数都用 `%d`，此处是唯一例外，现改正并加回归测试。
- **Agent 状态徽章只读文件夹是否存在**：于是出现了「未找到 · 15662 条记录」这种自相矛盾的行（Oh My Pi、Codex、Cline、DSH Harness 四行）。现为四态：正常 / 暂无记录 / 路径缺失（琥珀）/ 未找到，且零记录时不再重复显示「0 条记录」。
- **查询失败被渲染成 0**：Dashboard 的 `try?` 失败会让整条结论带变成「无使用记录」+ `$0.00` + `-100%` 环比；弹窗在 `summary == nil` 时显示「预估费用 $0.00 / 来源数 0」；Agent 分页在加载中显示「0 个 Agent 正常」；覆盖率分子取自整天 rollup，可对一个已经滚空的 24 小时窗口报「1 / 1」。现在失败保留上一次的值并标注 stale，未知显示「—」，一天窗口的覆盖率与取值同源。
- **失败也会盖上「刚刚更新」的时间戳**：`lastDataRefreshAt` 在读取失败时照样写，于是新鲜度胶囊转绿声称数据是新的。现在只有成功读取才盖戳；弹窗新增读取失败标记，失败时状态胶囊改说「数据可能已过期」。

### 同一件事不再说两遍、或说相反

- 侧栏的更新状态行与更新卡片逐字重复「当前版本 v1.6.0 · 上次检查：10 小时前」，且当状态为 `.idle` 时侧栏说「尚未检查」而卡片说 10 小时前 —— 现在侧栏只陈述**状态**（已是最新／发现新版本／检查失败／上次检查于 X）。
- 隐私声明在同一窗口出现 **3 次**（内容区页脚、通用页一行、关于页一张卡），而规范写的是「只出现一次」。现只保留页脚。
- 计价与汇率页两张卡的副标题直接把控件内容又写了一遍（「1 USD = 7.30 CNY」出现两次）；改为说明该设置的用途。
- 语言项显示成「跟随系统 (System Default)」——同一句两遍，其中一半还是英文。
- 趋势三件套的「均值」分母是**活跃桶**，但标签只写「均值」，读者用总量除以桶数会得到另一个数；改为「活跃均值」。

### 交互与控制

- **设置页的「立即同步」并不执行同步**：它只重新探测日志路径。改名为「重新检测」，弹窗里的真同步仍叫「立即同步」。
- **清空数据后占用空间不回落**：只统计主库文件，且 WAL 里的已释放页没有回收，于是「0 条记录 · 45 MB」看起来像清除失败。现在统计库+WAL+SHM 三个文件，并在清空后 `wal_checkpoint(TRUNCATE)`。
- **「重新聚合数据」不广播更新**：Dashboard 会一直显示重建前的数字。现在与「清空」一样发出 `bennettUsageDataDidUpdate`。
- **设置窗口长时间打开时数字冻结**：它不订阅数据更新通知。现在订阅（500 ms 去抖）。
- 年趋势浮层写死英文 `tokens`，金额用成功绿渲染（正是颜色角色分离要消灭的读法），图柱用**系统强调色**（用户把 macOS 强调色设成红色，年趋势就变成红色，而红色在本产品里是状态色）。三处都改为令牌与本地化文案。
- 热力图选中日期的横幅保存的是点击那一刻的值拷贝，后台刷新后它与同屏热力图不再同源；现在按 `dayKey` 重新绑定。
- 数据与存储页的副标题在英文下必然被截断（实测需 338pt，可用约 304pt）；现在允许换行。

### 数据层

- **Kimi 原地重写会双计**：wire 日志被压缩重写后适配器从 0 重扫，被改写行的 payload 变了 → 记录 id 变了 → 旧行留在库里。现在 `prefixHash` 变化也触发 cutover，先读完整快照再整体替换（协调器已有该机制，只是判据漏了这一个字段）。
- **Pi 的会话文件被截断/重写后永久不再被读取**：`guard fileSize > lastOffset` 让文件一旦变短就再也不会读，而若之后再增长又会从陈旧偏移续解析、把后半段归到错误记录上。Claude、Codex、Cline 都有从 0 重读的回退，Pi 漏了；现补上。

### 实测出来的排版缺陷（中英文长度差）

- 中文「主显示币种」分段控件需 **210pt** 却只给了 **150pt**：「人民币 CNY (¥)」右端被 ScrollView 裁掉并压在卡片边框上（英文只需 139pt，所以只在中文下出现）。改为 220pt。
- 英文「外观」分段控件需 **280.5pt** 却只给了 **200pt**：`Follow System` 被省略（中文只需 186pt）。改为 290pt。
- 弹窗「预估费用 / 来源数 / 状态」一行在英文下溢出 23–58pt（三个子项都是 `fixedSize`，无法压缩）：把新鲜度文案改短（`Data updated 59 mins ago` → `Updated 59 min ago` 等），实测最坏组合 **310.2pt ≤ 320pt**。
- 仪表盘只说「分钟」：`Data updated 1450 mins ago` 与弹窗同一时刻的 `Synced 1 days ago` 不一致；现在两处都按分/时/天递进。
- 三处 tooltip 仍写死英文 `tokens`；改为本地化的单位文案。
- 结论带的 stale 文案声称「以下为最近一次成功同步」，但它由**本地读取失败**驱动，且被传入一个它没有的占位符（参数被静默丢弃）；改为「读取本地数据库失败，以下为上次成功读取的数据」。首屏未加载完成时也不再先铺琥珀底。
- 模型名在趋势浮层与图例里都是中间省略且无处可看全名；补 `.help`。
- 项目榜展开时第 100 名的序号需 24.1pt 而只给了 20pt（13pt 等宽数字 8.03pt/字符）；改为 26pt。
- 中文「正常」只证明日志路径存在，不是健康检查；改为「已接入」（中英对齐事实）。
- 中文涨跌文案「+12% 较上一周期」语序读不通：把语序放进键（`%@ vs. previous period` / `较上一周期 %@`）。
- 外观选项硬编码中英字面量：第三语言永远翻译不到，且 `.themeSystem/.themeDark/.themeLight` 三个键成为死键；现在走本地化。
- 「前 12 个」丢了 `Top 12 of 100` 的分母口径；改为「前 12 / 100 个」。
- 删除死代码 `SidebarView`（写死英文、用系统 `accentColor`、仅被旧设计文档引用；连同它那两个只覆盖死代码的测试一起删）。

### 审计中未发现问题的部分（已核验）

- FSEvents 丢事件：5 个标志位全覆盖并走全量重扫，心跳每 5 分钟兑底，最多延迟、不会永久错数。
- 路径归一化：`/var` 与 `/private/var` 在游标键两侧都归一化，不构成漏读。
- 去重：内容寻址 id + 主键 + 批内 `seenIds` 下，同一 message.id 出现在多文件/多次解析不会双计（多计路径仅剩 id 漂移，即上面 Kimi/Pi 两条）。
- rollup 与原始记录：增量累加与全量重建表达式一致，未发现发散。
- 除零/NaN：全部除法点都有守卫，不会渲染出 `nan%`。

### 已知限制（本次未修，避免在补丁版里推行结构性改动）

- Pi 被**改写**（而非仅截断）的旧记录可能残留（id 含字节偏移）。结构解法是 Pi 迁移到 `fileGenerations` 游标并支持 cutover，代价是升级时一次全量重读（~1 GB），留待下个小版本。
- `day_key` 在解析入库时按当时时区固化：跨时区后热力图与 rollup 的「哪一天」可能偏移。今日口径已改为原始时间戳，因此同一屏的弹窗与看板不会再互相矛盾。
- 不在计价表内的模型静默按 $0 计费：规范里预留的「无价格表」状态还未实现（需先能区分「未知价格」与「真的免费」）。
- 「过去一年」的月度柱只画 12 个整月，而总额是滚动 365 天，首月残段存在于总额、不在图上（最多约 8%）。
- 清空后的磁盘回收仅做到 WAL 截断，未做 `VACUUM`（后者会重写整库文件）。

### 测试

新增 13 个回归测试（环比窗口同长度、今日同刻、部分覆盖、覆盖率同源、记录数渲染、7/30 天窗口边界、缓存命中率口径等），并删除 2 个只覆盖死代码的测试；共 **420 个测试全部通过**。
